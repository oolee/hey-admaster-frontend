...
import type { CanvasNodeMeta } from '@/skills/canvas-nodes';
import type { CanvasEdge, CanvasNode } from '@/types/canvas';
import type { VersionEntry } from '@/utils/version';
...
type DragState =
  | null
  | { fromId: string; fromPort: string; type: 'edge'; x?: number; y?: number }
  | { id: string; offsetX: number; offsetY: number; type: 'node' }
  | {
      origX: number;
      origY: number;
      startX: number;
  ...
  | { startX: number; startY: number; type: 'box' };
const dragging = ref<DragState>(null); // 节点拖动 / 画布平移 / 端口拉线 / 框选
...
interface ContextMenuItem {
  ...
  type?: string;
  isGroup?: boolean;
  children?: CanvasNodeMeta[];
}
const contextMenu = ref<{
  forNodeId: null | string;
  items: ContextMenuItem[];
  title: string;
  visible: boolean;
  x: number;
  y: number;
}>({ visible: false, x: 0, y: 0, items: [], title: '', forNodeId: null });
const portClickMenu = ref<{
  candidates?: CanvasNodeMeta[];
  nodeId: null | string;
  port: null | string;
  visible: boolean;
  x: number;
  y: number;
}>({ visible: false, x: 0, y: 0, nodeId: null, port: null });
const sidebarCollapsed = ref(false); // 任务 42：节点库折叠
const showAlbumModal = ref(false); // 任务 43：翻书 Modal
const albumNodeId = ref<null | string>(null);
...
function screenToWorld(sx: number, sy: number) {
  if (!canvasRef.value) return { wx: 0, wy: 0 };
...
/* ===== 端口位置 ===== */
function portPos(nodeId: string, type: string) {
...
function edgePath(e: CanvasEdge) {
...
function edgeMid(e: CanvasEdge) {
  const a = portPos(e.from, 'out');
  const b = portPos(e.to, 'in');
  return { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
}
function edgeColor(e: CanvasEdge) {
  const fromNode = store.nodes.find((n) => n.id === e.from);
  const meta = fromNode ? nodeMeta(fromNode.type) : null;
  return PORT_COLOR[meta?.produces ?? 'any'] || PORT_COLOR.any;
}
...
function onMouseDown(e: MouseEvent) {
  ...
  // 端口：拖动连接线
  const portAttr = target.dataset?.port;
  if (portAttr) {
    const [nodeId, port] = portAttr.split(':');
...
const dragHover = ref<null | { nodeId: string; type: 'invalid' | 'valid' }>(
  null,
);
...
function onMouseMove(e: MouseEvent) {
  ...
  if (d.type === 'edge') {
    ...
    if (portEl?.dataset?.port) {
      ...
      if (toId && toId !== d.fromId && toPort === 'in') {
        dragHover.value = {
...
function onMouseUp(e: MouseEvent) {
  ...
  if (dragging.value.type === 'edge') {
    const portEl = document
      ...
      ?.closest?.('[data-port]') as HTMLElement | null;
    if (portEl?.dataset?.port) {
      const [toId, toPort] = portEl.dataset.port.split(':');
      if (toId && toPort === 'in' && toId !== dragging.value.fromId) {
...
function dragEdgePath() {
  ...
  const ex = (dragging.value.x ?? 0) - rect.left;
  const ey = (dragging.value.y ?? 0) - rect.top;
...
/* ===== 端口单击：弹下游节点菜单 ===== */
function onPortClick(e: MouseEvent, nodeId: string, port: string) {
  e.stopPropagation();
  e.preventDefault();
  // 仅输出端口点击有意义（用户想看下游）
  if (port !== 'out') return;
  ...
  const produces = srcMeta?.produces;
  if (!produces) return;
  const candidates = CANVAS_NODES.filter(
    (n) => n.accepts.includes(produces) || n.accepts.includes('any'),
  );
  portClickMenu.value = {
    visible: true,
    x: e.clientX,
...
function pickFromPortClick(candidate: CanvasNodeMeta) {
...
function onContextAction(action: ContextMenuItem) {
...
function clearCanvas() {
  if (!store.nodeCount || confirm('确认清空画布？')) {
    store.clear();
    toast.info('画布已清空');
  }
}
...
let autoVersionTimer: null | ReturnType<typeof setTimeout> = null;
...
async function loadVer(id: string) {
...
async function removeVer(id: string, e: MouseEvent) {
  e.stopPropagation();
...
function updateTextInput(id: string, text: string) {
  store.updateNodeParams(id, { text });
}

/* 入口节点检测（无输入边），用于显示"等待上游"提示 */
function hasUpstream(nodeId: string) {
...
const boxSelectRect = ref<null | {
  ex: number;
  ey: number;
  sx: number;
  sy: number;
}>(null); // { sx, sy, ex, ey } 屏幕坐标
...
function setMultiSelect(ids: string[]) {
  selectedIds.value = ids;
}
function isInBox(node: CanvasNode) {
...
/* ===== 浮层内下拉开关 ===== */
const openPanel = ref<null | string>(null);
function closeAllPanels() {
  openPanel.value = null;
}

function onModelPick(opt: string) {
  store.updateNodeParams(store.selectedId, { model: opt });
  openPanel.value = null;
}
function onSizePick(s: string) {
  store.updateNodeParams(store.selectedId, { size: s });
  openPanel.value = null;
}
function onDurPick(d: number) {
...
function isComplexNode(type: string) {
...
function isVisualNode(type: string) {
...
function copyText(node: CanvasNode) {
...
function downloadOne(node: CanvasNode) {
  const result = node.result;
...
function triggerExport(node: CanvasNode) {
  try {
    // 收集上游节点的产物信息
    ...
    upstream.forEach((u) => {
      parts.push(`【${u.label}】`);
      if (u.result && 'url' in u.result && u.result.url)
        parts.push(`  产物：${u.result.url}`);
      if (u.result?.type === 'text' && u.result.text)
        parts.push(`  文本：${u.result.text.slice(0, 80)}`);
      if (u.result?.type === 'quote' && u.result.total)
        parts.push(`  报价：¥${u.result.total}`);
      if (u.result?.type === 'material' && u.result.items)
        parts.push(`  物料：${u.result.items.length} 项`);
      parts.push('');
    });
...
function copyImage(node: CanvasNode) {
  const result = node.result;
  if (!result || !('url' in result) || !result.url) return;
  try {
    navigator.clipboard?.writeText(`[${node.label}] ${result.url}`);
...
/* 模型选项按节点 LLM 类型分组 */
function modelOptionsFor(type: string) {
...
function presetsFor(type: string) {
  return MODE_PRESETS;
}

/* 节点操作面板 helper（每个节点独立 panel key） */
function pickModel(node: CanvasNode, opt: string) {
  store.updateNodeParams(node.id, { model: opt });
  openPanel.value = null;
}
function pickSize(node: CanvasNode, s: string) {
  store.updateNodeParams(node.id, { size: s });
  openPanel.value = null;
}
function pickDur(node: CanvasNode, d: number) {
  store.updateNodeParams(node.id, { duration: d });
  openPanel.value = null;
}
function pickQuality(node: CanvasNode, q: string) {
  store.updateNodeParams(node.id, { quality: q });
  openPanel.value = null;
}
function pickPreset(node: CanvasNode, p: string) {
  store.updateNodeParams(node.id, { preset: p });
  openPanel.value = null;
}
function createUpstreamInput(node: CanvasNode, type: string) {
  if (!node) return;
...
function alignSelected(mode: string) {
  ...
  const wOf = (t: string) => nodeMeta(t)?.w ?? 240;
  const hOf = (t: string) => nodeMeta(t)?.h ?? 140;
  if (mode === 'left') {
    const min = Math.min(...nodes.map((n) => n.x));
    nodes.forEach((n) => (n.x = min));
  }
  if (mode === 'right') {
    const max = Math.max(...nodes.map((n) => n.x + wOf(n.type)));
    nodes.forEach((n) => (n.x = max - wOf(n.type)));
  }
  if (mode === 'top') {
  ...
  if (mode === 'bottom') {
    const max = Math.max(...nodes.map((n) => n.y + hOf(n.type)));
    nodes.forEach((n) => (n.y = max - hOf(n.type)));
  }
  if (mode === 'center-h') {
    const avg =
      nodes.reduce((s, n) => s + n.x + wOf(n.type) / 2, 0) / nodes.length;
    nodes.forEach((n) => (n.x = avg - wOf(n.type) / 2));
  }
  if (mode === 'center-v') {
    const avg =
      nodes.reduce((s, n) => s + n.y + hOf(n.type) / 2, 0) / nodes.length;
    nodes.forEach((n) => (n.y = avg - hOf(n.type) / 2));
  }
...
function downloadSelected() {
  ...
  ids.forEach((id) => {
    ...
    if (!n?.result) return;
    if (
      (n.result.type === 'image' || n.result.type === 'video') &&
      n.result.url
    )
      images.push({ name: `${n.label}.svg`, url: n.result.url });
    if (n.result.type === 'album' && n.result.pages)
...
function openAlbumModal(node: CanvasNode) {
...
function setAlbumMode(m: string) {
  albumMode.value = m;
...
function onArtFrameClick(n: CanvasNode) {
...
function onNodePickFromMenu(type: string) {
  if (!canvasRef.value) return;
  const rect = canvasRef.value.getBoundingClientRect();
...
function sliderMin(key: string) {
  return SLIDER_META[key]?.min ?? 0;
}
function sliderMax(key: string) {
  return SLIDER_META[key]?.max ?? 100;
}
function sliderStep(key: string) {
  return SLIDER_META[key]?.step ?? 1;
}

/* ===== 模板辅助：事件取值 / 结果收窄 ===== */
function evValue(e: Event): string {
  return (e.target as HTMLInputElement).value;
}
function resultUrl(node: CanvasNode): string {
  if (!node.result) return '';
  return 'url' in node.result && node.result.url ? node.result.url : '';
}
function resultText(node: CanvasNode): string {
  return node.result?.type === 'text' ? node.result.text : '';
}
function resultPages(node: CanvasNode): string[] {
  return node.result?.type === 'album' && node.result.pages
    ? node.result.pages
    : [];
}
function resultShareId(node: CanvasNode): string {
  return node.result?.type === 'share' && node.result.shareId
    ? node.result.shareId
    : '';
}
function updateResultText(node: CanvasNode, text: string) {
  if (node.result && node.result.type === 'text') node.result.text = text;
}
...
onBeforeUnmount(() => {
  ...
  clearTimeout(autoVersionTimer ?? undefined);
});
    ...
    <header class="ic-top">
      <div class="ic-top-left">
        ...
        <input
          ...
          @change="store.projectName = evValue($event)"
          placeholder="项目名称"
          title="项目名称（保存到云端）"
        />
    ...
    <div class="ic-body">
      ...
      >
        ...
        >
          ...
          >
            ...
            <div v-if="store.selectedId !== n.id" class="node-summary">
              ...
              <template
                v-else-if="
                  (nodeMeta(n.type)?.inputs ?? 0) > 0 && !hasUpstream(n.id)
                "
              >
            ...
            <template v-else>
              ...
              <div class="node-main">
                ...
                <template v-if="isVisualNode(n.type)">
                  ...
                  <div
                    v-else-if="n.status === 'done' && resultUrl(n)"
                ...
                <template v-else-if="isTextNode(n.type)">
                  <div
                    v-if="n.status === 'done' && resultText(n)"
                    class="text-result"
                  >
                    <div class="tr-head">
                    ...
                    <textarea
                      ...
                      rows="4"
                      :value="resultText(n)"
                      @input="updateResultText(n, evValue($event))"
                    ></textarea>
                  </div>
                  <div
                ...
                >
                  <textarea
                    ...
                    :value="String(n.params.text ?? '')"
                    @input="
                      store.updateNodeParams(n.id, { text: evValue($event) })
                    "
                  ></textarea>
                </template>
                ...
                <template v-else-if="n.type === 'image-input'">
                  <div v-if="resultUrl(n)" class="visual-state done">
                    <img :src="resultUrl(n)" alt="" class="result-img" />
                  </div>
                  <div v-else class="visual-state idle">
                ...
                <template v-else-if="n.type === 'req-form'">
                  <div class="req-form">
                    <div class="rf-row">
                      ...
                      <input
                        :value="String(n.params.customer ?? '')"
                        placeholder="如：王姐奶茶店"
                        @input="
                          store.updateNodeParams(n.id, {
                            customer: evValue($event),
                          })
                        "
                      />
                    </div>
                    <div class="rf-row">
                      ...
                      <select
                        :value="String(n.params.projectType ?? '')"
                        @change="
                          store.updateNodeParams(n.id, {
                            projectType: evValue($event),
                          })
                        "
                      >
                        <option>门头店招</option>
                    ...
                    <div class="rf-grid">
                      ...
                      <div class="rf-row">
                        ...
                        <input
                          :value="String(n.params.budget ?? '')"
                          placeholder="5000"
                          @input="
                            store.updateNodeParams(n.id, {
                              budget: evValue($event),
                            })
                          "
                        />
                      </div>
                    ...
                    <div class="rf-row">
                      ...
                      <input
                        :value="String(n.params.material ?? '')"
                        placeholder="铝塑板+灯带"
                        @input="
                          store.updateNodeParams(n.id, {
                            material: evValue($event),
                          })
                        "
                      />
                    </div>
                    <div class="rf-row">
                      ...
                      <input
                        :value="String(n.params.deadline ?? '')"
                        placeholder="5 天"
                        @input="
                          store.updateNodeParams(n.id, {
                            deadline: evValue($event),
                          })
                        "
                      />
                    </div>
                  ...
                  <div v-if="resultText(n)" class="brief-card">
                    <p class="bc-title">📋 需求摘要</p>
                    <pre class="bc-text">{{ resultText(n) }}</pre>
                  </div>
                </template>
            ...
            <!-- 端口 -->
            <span
              v-if="(nodeMeta(n.type)?.inputs ?? 0) > 0"
              class="port port-in"
              :style="{
                '--pc':
                  PORT_COLOR[nodeMeta(n.type)?.accepts[0] ?? 'any'] ||
                  PORT_COLOR.any,
              }"
              :data-port="`${n.id}:in`"
            ...
            ></span>
            <span
              v-if="(nodeMeta(n.type)?.outputs ?? 0) > 0"
              class="port port-out"
              :style="{
                '--pc':
                  PORT_COLOR[nodeMeta(n.type)?.produces ?? 'any'] ||
                  PORT_COLOR.any,
              }"
              :data-port="`${n.id}:out`"
        ...
        >
          ...
          <g
            v-if="dragging"
            :transform="`translate(${(dragging.x ?? 0) - (canvasRef?.getBoundingClientRect()?.left ?? 0)}, ${(dragging.y ?? 0) - (canvasRef?.getBoundingClientRect()?.top ?? 0)})`"
          >
            <circle
              r="10"
              :fill="
        ...
        <template v-if="store.selected">
          ...
          <label class="ins-field">
            ...
            <input
              ...
              @input="store.updateNodeLabel(store.selected.id, evValue($event))"
            />
          </label>
          ...
          >
            <span>推荐模型</span>
            <select
              :value="String(store.selected.params.model ?? '')"
              @change="
          ...
          <label v-if="store.selected.type === 'image-gen'" class="ins-field">
            <span>图像尺寸</span>
            <select
              :value="String(store.selected.params.size ?? '')"
              @change="
                store.updateNodeParams(store.selected.id, {
                  size: evValue($event),
                })
          ...
          <label v-if="store.selected.type === 'text-input'" class="ins-field">
            ...
            <textarea
              rows="3"
              :value="String(store.selected.params.text ?? '')"
              @input="updateTextInput(store.selected.id, evValue($event))"
            ></textarea>
          </label>
          ...
          >
            <span>生图模式预设</span>
            <select
              :value="String(store.selected.params.preset ?? '')"
              @change="
                store.updateNodeParams(store.selected.id, {
                  preset: evValue($event),
                })
              "
            >
    ...
    <Teleport to="body">
      <Transition name="ctx">
        ...
        >
          ...
          <template v-else>
            <div
              v-for="group in contextMenu.items"
              :key="group.type"
              class="ctx-group"
            >
            ...
            <div class="book-wrap">
              ...
              <div class="book" :class="albumMode">
                ...
                >
                  ...
                  >
                    <img
                      :src="`/asset/mock-cover-${COVERS[albumPage]}.svg`"
                      :alt="`第 ${albumPage + 1} 页`"
                    />
                ...
                <Transition name="book-double" mode="out-in">
                  ...
                  >
                    ...
                    <div class="book-page right">
                      <img
                        :src="`/asset/mock-cover-${COVERS[albumPage + 1] || COVERS[albumPage]}.svg`"
                        :alt="`第 ${albumPage + 2} 页`"
                      />
...
.book-single-fwd-enter-active {
  transform-origin: left center;
  animation: page-flip-fwd 0.7s var(--ease-spring) both;
}

.book-single-fwd-leave-active {
  transform-origin: left center;
  animation: page-flip-fwd-out 0.7s var(--ease-spring) both;
}

@keyframes page-flip-fwd {
  0% {
    opacity: 0;
...
.book-double-enter-active {
  animation: book-open 0.6s var(--ease-spring) both;
}

.book-double-leave-active {
  animation: book-open-out 0.4s var(--ease-spring) both;
}
