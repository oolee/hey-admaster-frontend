...
import { Plus, Eye, Pencil, Trash2, Heart } from 'lucide-vue-next'
...
onMounted(async () => {
  // 复用前台案例数据（简化版）
  cases.value = [
    { id: 1, title: '山野菜篮子 · 餐饮全案', category: '门头店招', industry: '餐饮连锁', views: 3280, likes: 486, status: 'published' },
    { id: 2, title: '构建商业 · 主视觉系统', category: 'VI 设计', industry: '科技互联网', views: 2465, likes: 392, status: 'published' },
    { id: 3, title: '潮玩品牌 · 社媒矩阵', category: '社媒内容', industry: '新消费', views: 1987, likes: 312, status: 'published' },
    { id: 4, title: '普者黑 · 旅游海报', category: '海报设计', industry: '文旅', views: 3120, likes: 428, status: 'published' },
    { id: 5, title: '少儿编程 · 招生传单', category: '印刷物料', industry: '教育', views: 1654, likes: 201, status: 'draft' },
    { id: 6, title: '草本护肤 · Logo 设计', category: 'Logo 设计', industry: '美妆', views: 2891, likes: 356, status: 'published' },
    { id: 7, title: '夏季音乐节 · 主视觉', category: '海报设计', industry: '活动', views: 3756, likes: 512, status: 'published' }
  ]
})
</script>
...
<template>
  <div class="pg">
    ...
    <div class="card table-card">
      <table class="tbl">
        <thead><tr><th>案例</th><th>分类</th><th>行业</th><th>浏览</th><th>点赞</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="c in cases" :key="c.id">
            <td class="strong">{{ c.title }}</td>
            <td><ABadge tone="accent">{{ c.category }}</ABadge></td>
            <td class="muted">{{ c.industry }}</td>
            <td class="num">{{ c.views.toLocaleString() }}</td>
            <td class="num like"><Heart :size="12" /> {{ c.likes }}</td>
            <td><ABadge :tone="c.status === 'published' ? 'success' : 'warning'">{{ c.status === 'published' ? '已发布' : '草稿' }}</ABadge></td>
            <td>
              <div class="row-actions">
                <button class="ra" title="预览"><Eye :size="14" /></button>
                <button class="ra" title="编辑"><Pencil :size="14" /></button>
                <button class="ra danger" title="删除" @click="toast.info('演示：删除案例')"><Trash2 :size="14" /></button>
              </div>
            </td>
          </tr>
        </tbody>
...
.num.like { display: inline-flex; align-items: center; gap: 4px; color: var(--color-error); }
