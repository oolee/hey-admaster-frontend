...
const filtered = computed(() => {
  ...
  if (kw) list = list.filter((o) => String(o.user || '').toLowerCase().includes(kw) || String(o.id || '').toLowerCase().includes(kw))
  return list
})
const totalRevenue = computed(() => orders.value.filter((o) => o.status === 'paid').reduce((s, o) => s + Number(o.amount || 0), 0))
  ...
  <div class="pg">
    <header class="pg-head">
      <div>
        ...
        <p class="pg-sub">已支付金额合计 ¥{{ totalRevenue.toLocaleString() }}</p>
      </div>
    </header>
    ...
    <div class="toolbar">
      ...
      <div class="filter-tabs">
        ...
        <button :class="{ on: statusFilter === 'paid' }" @click="statusFilter = 'paid'">已支付</button>
        <button :class="{ on: statusFilter === 'pending' }" @click="statusFilter = 'pending'">待支付</button>
        <button :class="{ on: statusFilter === 'refunded' }" @click="statusFilter = 'refunded'">已退款</button>
      </div>
    </div>
    ...
    <div class="card table-card">
      <table class="tbl">
        <thead>
          <tr><th>订单号</th><th>用户</th><th>商品</th><th>金额</th><th>支付方式</th><th>时间</th><th>状态</th></tr>
        </thead>
        <tbody>
          <tr v-for="o in filtered" :key="o.id">
            ...
            <td><ABadge :tone="statusMetaOf(o.status).tone">{{ statusMetaOf(o.status).label }}</ABadge></td>
          </tr>
        </tbody>
      </table>
...
.search-box { display: flex; align-items: center; gap: 8px; padding: 0 12px; height: 38px; background: var(--color-surface); border: 1.5px solid var(--color-border); border-radius: 10px; color: var(--color-text-3); min-width: 240px; }
