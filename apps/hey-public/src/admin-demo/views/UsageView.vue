...
import { Wallet, TrendingUp, DollarSign, PieChart } from 'lucide-vue-next'
...
<template>
  <div class="pg">
    ...
    <div class="chart-grid">
      <div class="card">
        <div class="card-head"><h3>模型调用分布</h3><span class="card-hint">近 30 日</span></div>
        <MiniChart :points="data.usage.map(u => ({ label: u.model.split(' ')[0], value: u.calls }))" type="bar" />
      </div>
...
.profit-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px; margin-bottom: 20px; }
@media (min-width: 1024px) { .profit-grid { grid-template-columns: repeat(4, 1fr); } }
.profit-card {
...
.p-main { display: flex; flex-direction: column; flex: 1; }
.p-value { font-family: var(--font-display); font-size: var(--text-lg); font-weight: 700; color: var(--color-text-1); }
.p-label { font-size: var(--text-xs); color: var(--color-text-3); }
.p-note { font-size: var(--text-xs); color: var(--color-text-2); font-weight: 500; }

.chart-grid { display: grid; grid-template-columns: 1fr; gap: 16px; }
@media (min-width: 1024px) { .chart-grid { grid-template-columns: 1.3fr 1fr; } }
...
.usage-row { display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px dashed var(--color-border); }
.usage-row:last-child { border-bottom: none; }
.u-name { flex: 1; font-weight: 600; color: var(--color-text-1); font-size: var(--text-sm); }
.u-calls { font-family: var(--font-display); font-weight: 600; font-size: var(--text-sm); }
.u-tokens { font-size: var(--text-xs); color: var(--color-text-3); }
</style>