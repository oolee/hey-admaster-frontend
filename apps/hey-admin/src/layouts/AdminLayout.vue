...
const groups = [
  {
    label: '总览',
    items: [{ path: '/dashboard', label: '数据看板', icon: LayoutDashboard }],
  },
  {
    label: '经营',
    items: [
      ...
      { path: '/credits', label: '积分与计费', icon: Coins },
    ],
  },
  {
    label: '模型',
    items: [
      ...
      { path: '/skills', label: '技能管理', icon: Sparkles },
      { path: '/usage', label: '用量与利润', icon: BarChart3 },
      { path: '/records', label: '调用记录', icon: History },
    ],
  },
  {
    label: '内容',
    items: [
      ...
      { path: '/templates', label: '模板管理', icon: LayoutGrid },
    ],
  },
  {
    label: '审核',
    items: [{ path: '/audit', label: '内容审核', icon: ShieldCheck }],
  },
  {
    label: '系统',
    ...
    <aside class="al-side" :class="{ 'mobile-open': mobileOpen }">
      ...
      <nav class="al-nav">
        <div v-for="g in groups" :key="g.label" class="al-group">
          ...
          >
            <item.icon :size="17" />
            <span v-if="!collapsed" class="al-item-label">{{
              item.label
            }}</span>
          </router-link>
        </div>
          ...
          <div class="al-user-wrap">
            <button class="al-user" @click="userMenuOpen = !userMenuOpen">
              ...
              <span class="al-user-info">
                <span class="al-user-name">{{
                  admin.admin?.name || '管理员'
                }}</span>
            ...
            <Transition name="pop">
              <div v-if="userMenuOpen" class="al-user-menu" @click.stop>
                <div class="al-um-head">
                  <span class="al-um-avatar">{{
                    admin.admin?.avatar || 'A'
                  }}</span>
                  <div>
                    <p class="al-um-name">
                      {{ admin.admin?.name || '管理员' }}
