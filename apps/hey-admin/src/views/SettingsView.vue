  ...
  <div class="pg">
    <header class="pg-head">
      ...
      <div class="pg-actions">
        <AButton variant="outline" @click="reset">
    ...
    <div v-if="form" class="settings-grid">
      <div v-for="g in groups" :key="g.key" class="card">
        ...
        <template v-if="g.key === 'site'">
          <label class="sf"
            ><span>站点名称</span><input v-model="form.site.name"
          /></label>
          <label class="sf"
            ><span>站点标语</span><input v-model="form.site.slogan"
          /></label>
          <label class="sf"
            ><span>客服邮箱</span><input v-model="form.site.contact"
          /></label>
        </template>
        <template v-else-if="g.key === 'security'">
          <label class="sf"
            ><span>登录失败限制</span
            ><input
              v-model.number="form.security.loginLimit"
              type="number"
              min="1"
              max="20"
          /></label>
          <label class="sf switch"
            ><span>短信验证码登录</span
            ><input v-model="form.security.smsVerify" type="checkbox"
          /></label>
          <label class="sf switch"
            ><span>内容自动审核</span
            ><input v-model="form.security.auditContent" type="checkbox"
          /></label>
        </template>
        <template v-else-if="g.key === 'billing'">
          <label class="sf"
            ><span>税率（%）</span
            ><input
              v-model.number="form.billing.taxRate"
              type="number"
              min="0"
              max="30"
          /></label>
          <label class="sf"
            ><span>发票前缀</span><input v-model="form.billing.invoicePrefix"
          /></label>
          <label class="sf switch"
            ><span>自动续费</span
            ><input v-model="form.billing.autoRenew" type="checkbox"
          /></label>
        </template>
        <template v-else>
          <label class="sf">
          ...
          <label class="sf"
            ><span>并发生成上限</span
            ><input
              v-model.number="form.ai.concurrent"
              type="number"
              min="1"
              max="16"
          /></label>
