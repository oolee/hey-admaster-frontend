      ...
      <div v-for="p in data.plans" :key="p.id" class="plan-card">
        <div class="plan-top">
          ...
          <span class="plan-price">{{
            p.price == null ? '定制' : `¥${Number(p.price).toLocaleString()}`
          }}</span>
        </div>
        <p class="plan-desc">{{ p.desc || p.description || '' }}</p>
        <p class="plan-credits">
          <template v-if="p.credits">
            {{ Number(p.credits).toLocaleString() }} 积分 /
            {{ (p.desc || '').includes('年') ? '年' : '期' }}
          </template>
          <template v-else>按需报价</template>
        </p>
        <button class="plan-edit" @click="toast.info('演示：编辑套餐')">
          编辑
        </button>
      </div>
      <button class="plan-add" @click="toast.info('演示：新增套餐')">
        <Plus :size="20" /> 新增套餐
      </button>
    </div>
      ...
      <table class="tbl">
        ...
        <tbody>
          <tr v-for="l in data.logs" :key="l.id">
            ...
            <td>
              <ABadge :tone="logTone[l.type]">
                {{
                  { consume: '消耗', recharge: '充值', refund: '退还' }[l.type]
                }}
              </ABadge>
            </td>
            <td class="num" :class="l.amount < 0 ? 'neg' : 'pos'">
              {{ l.amount > 0 ? '+' : '' }}{{ l.amount }}
            </td>
