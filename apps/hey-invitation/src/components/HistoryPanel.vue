...
const reversedHistory = computed(() => {
  const list: { label: string; realIndex: number; timestamp: number }[] = [];
  ...
  <Transition name="history-panel">
    <div v-if="store.historyPanelVisible" class="history-panel-container">
      <div
        ...
        @click="store.setHistoryPanelVisible(false)"
      ></div>
      <div class="history-panel">
        <div class="history-panel-header">
          ...
          <div class="history-panel-actions">
            <!-- prettier-ignore -->
            <span class="history-info">共 {{ store.history.length }} 步 (当前:
              {{ store.historyIndex + 1 }})</span>
            <button
        ...
        <div class="history-panel-list">
          ...
          >
            <div class="history-item-icon">
              <i
                v-if="item.realIndex === store.historyIndex"
                class="bi bi-dot"
              ></i>
              <i
                v-else-if="item.realIndex < store.historyIndex"
