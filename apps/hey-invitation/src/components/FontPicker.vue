<template>
  <div class="font-picker" ref="containerRef">
    <button type="button" class="font-picker-btn" @click="togglePanel" :style="{ fontFamily: modelValue }">
      ...
      <div v-if="panelVisible" class="font-panel" @click.stop>
        ...
        <div class="font-panel-list">
          ...
          <div
            ...
            :key="font.family"
            class="font-item"
            :class="{ active: font.family === modelValue, 'font-user': font.type === 'user' }"
            @click="selectFont(font)"
          >
            <div class="font-item-left">
              <span class="font-item-name" :style="{ fontFamily: font.family }">{{ font.name }}</span>
              <span class="font-item-badge" :class="'badge-' + font.type">
                {{ font.type === 'user' ? '用户字体' : font.type === 'google' ? '免费商用' : '系统' }}
              </span>
            </div>
            <div class="font-item-right">
              <span v-if="font.license" class="font-item-license">{{ font.license }}</span>
              <i v-if="font.family === modelValue" class="bi bi-check2 font-item-check"></i>
              <button
                v-if="font.type === 'user'"
        ...
        <div class="font-panel-footer">
          ...
          <button type="button" class="btn btn-sm btn-outline-primary font-upload-btn" @click="onUploadClick">
            <i class="bi bi-upload"></i> 上传字体
          </button>
          <input type="file" ref="fileInputRef" accept=".ttf,.otf,.woff,.woff2" class="font-file-input" @change="onFileSelected" />
        </div>
      </div>
    </Transition>
    ...
    <Transition name="modal">
      <div v-if="licenseDialogVisible" class="license-modal-mask" @click="licenseDialogVisible = false">
        <div class="license-modal" @click.stop>
          <div class="license-modal-header">
            <span class="license-modal-title">
              <i class="bi bi-shield-lock-fill"></i> 字体使用授权声明
            </span>
            <button type="button" class="btn-close-small" @click="licenseDialogVisible = false">
              <i class="bi bi-x"></i>
            </button>
          </div>
          <div class="license-modal-body">
            <div class="license-notice">
              ...
              <ol>
                <li>您上传的字体必须为您个人合法拥有或已取得合法授权的字体文件。</li>
                <li>本平台不会对您上传字体的版权归属做任何审核或担保，也不承担任何因字体侵权产生的法律责任。</li>
                <li>如果您是商用目的（包括婚礼请柬用于商业场景），请确保字体的商业使用已获得版权方许可。</li>
                <li>您上传的字体文件仅在您的浏览器本地存储，不会上传到任何服务器。</li>
                <li>如因上传未授权商用字体导致版权方追究责任，责任由您个人全部承担。</li>
              </ol>
              <p><strong>推荐使用免费商用字体（如 Google Fonts 中的 Noto Sans SC 等），可直接在列表中选用，无需上传。</strong></p>
            </div>
          ...
          <div class="license-modal-footer">
            <button type="button" class="btn btn-sm btn-outline-secondary" @click="licenseDialogVisible = false">取消</button>
            <button type="button" class="btn btn-sm btn-primary" :disabled="!licenseAgreed" @click="onConfirmUpload">
...
const filteredFonts = computed(() => {
  ...
  return all.filter((f) => f.name.toLowerCase().includes(q) || f.family.toLowerCase().includes(q));
});
...
const onDeleteUserFont = (family: string) => {
  if (confirm('确定删除该字体吗？删除后如其他元素仍引用该字体，将回退为默认字体。')) {
