  ...
  <aside class="editor-sidebar sidebar-right">
    <div class="sidebar-tabs">
      ...
      <div class="tab-content">
        <!-- 属性设置 -->
        <div v-if="store.rightPanelTab === 'properties'" class="tab-pane active" id="tabProperties" role="tabpanel">
                    <!-- 音乐管理（可折叠） -->
          <div class="collapsible-section">
            <div class="section-header" @click="toggleCollapse('music')">
              <h6 class="section-title">音乐管理</h6>
              <i class="bi" :class="collapsed.music ? 'bi-chevron-right' : 'bi-chevron-down'"></i>
            </div>
            <div v-if="!collapsed.music" class="section-body">
              ...
              <div class="music-progress-group">
                ...
                <div class="d-flex align-items-center">
                  ...
                  >
                  <span class="form-value ms-2">{{ formatTime(currentTime) }}/{{ formatTime(duration) }}</span>
                </div>
              </div>

              <div class="music-action-group">
                <label class="form-label-sm block-label">操作</label>
                <div class="btn-grid" style="display:flex;flex-direction:column;gap:4px;">
                  <div style="display:flex;gap:4px;">
                    <button class="btn btn-primary btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" @click="onAddMusic">
                      <i class="bi bi-music-note"></i> 添加
                    </button>
                    <button class="btn btn-outline-secondary btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" :disabled="!hasAudio" @click="onChangeMusic">
                      <i class="bi bi-music-note-beamed"></i> 更换
                    </button>
                  </div>
                  <div style="display:flex;gap:4px;">
                    <button class="btn btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" :disabled="!hasAudio" @click="onTogglePlayPause">
                      <i :class="isPlaying ? 'bi bi-pause-fill' : 'bi bi-play-fill'"></i>
                      {{ isPlaying ? "暂停" : "播放" }}
                    </button>
                    <button class="btn btn-outline-secondary btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" :disabled="!hasAudio" @click="onResetPosition">
                      <i class="bi bi-arrow-repeat"></i> 复位
                    </button>
                  </div>
                  <div style="display:flex;gap:4px;">
                    <button class="btn btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" :class="store.audio.locked ? 'btn-outline-success' : 'btn-outline-secondary'" :disabled="!hasAudio" @click="onToggleLock">
                      <i :class="store.audio.locked ? 'bi bi-unlock' : 'bi bi-lock'"></i>
                      {{ store.audio.locked ? "解锁" : "锁定" }}
                    </button>
                    <button class="btn btn-outline-danger btn-sm action-btn" style="font-size:10px;padding:1px 8px;flex:1;" :disabled="!hasAudio" @click="onDeleteMusic">
            ...
            <div class="music-icon-settings">
              ...
              <div class="icon-grid">
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-music-note' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-music-note')" title="音符"><i class="bi bi-music-note"></i></button>
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-music-note-beamed' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-music-note-beamed')" title="双音符"><i class="bi bi-music-note-beamed"></i></button>
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-vinyl' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-vinyl')" title="唱片"><i class="bi bi-vinyl"></i></button>
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-disc' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-disc')" title="光盘"><i class="bi bi-disc"></i></button>
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-boombox' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-boombox')" title="音响"><i class="bi bi-boombox"></i></button>
                <button class="btn btn-sm" :class="store.audio.iconClass === 'bi bi-speaker' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioIcon('bi bi-speaker')" title="喇叭"><i class="bi bi-speaker"></i></button>
              </div>
              <label class="form-label-sm block-label mt-2">动画效果</label>
              <div class="btn-group btn-group-sm w-100">
                <button class="btn btn-sm" :class="store.audio.animation === 'spin' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioAnimation('spin')">旋转</button>
                <button class="btn btn-sm" :class="store.audio.animation === 'pulse' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioAnimation('pulse')">脉冲</button>
                <button class="btn btn-sm" :class="store.audio.animation === 'bounce' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioAnimation('bounce')">弹跳</button>
                <button class="btn btn-sm" :class="store.audio.animation === 'blink' ? 'btn-primary' : 'btn-outline-secondary'" @click="store.setAudioAnimation('blink')">闪烁</button>
              </div>
            </div>
          </div>
          ...
          <div class="collapsible-section">
            <div class="section-header" @click="toggleCollapse('page')">
              ...
              <i class="bi" :class="collapsed.page ? 'bi-chevron-right' : 'bi-chevron-down'"></i>
            </div>
          ...
          <div class="collapsible-section">
            <div class="section-header" @click="toggleCollapse('text')">
              ...
              <i class="bi" :class="collapsed.text ? 'bi-chevron-right' : 'bi-chevron-down'"></i>
            </div>
            <div v-if="!collapsed.text" class="section-body">
              ...
              <div class="form-group">
                ...
                <div class="d-flex align-items-center">
                  <input type="range" class="form-range flex-grow-1" min="12" max="72" value="24">
                  <span class="form-value ms-2" style="min-width: 40px;">24px</span>
                </div>
              </div>
              <div class="form-group">
                <label class="form-label-sm block-label">颜色</label>
                <input type="color" class="form-control form-control-color" value="#d63384">
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 页面管理 -->
        <div v-if="store.rightPanelTab === 'pages'" class="tab-pane active" id="tabPages" role="tabpanel">
          <div class="pages-manager">
            <div class="pages-toolbar mb-3">
              <div class="d-flex gap-1 mb-2">
                <button class="btn btn-sm btn-outline-primary flex-grow-1" @click="onAddPage">
                  <i class="bi bi-plus-lg"></i> 添加
                </button>
                <button class="btn btn-sm btn-outline-secondary flex-grow-1" @click="onCopyCurrentPage">
                  <i class="bi bi-files"></i> 复制
                </button>
              </div>
              <div class="d-flex gap-1 align-items-center justify-content-between">
                <button class="btn btn-sm btn-outline-secondary flex-grow-1" :disabled="store.currentPageIndex <= 0" @click="onPrevPage">
                  <i class="bi bi-chevron-left"></i> 上一页
                </button>
                <span class="page-indicator">{{ store.currentPageIndex + 1 }}/{{ store.pages.length }}</span>
                <button class="btn btn-sm btn-outline-secondary flex-grow-1" :disabled="store.currentPageIndex >= store.pages.length - 1" @click="onNextPage">
            ...
            <div class="pages-list">
              <div
                ...
                :key="page.id"
                class="page-item"
                :class="{ active: store.currentPageId === page.id, dragging: dragPageFrom === index, 'drag-over': dragPageOver === index }"
                draggable="true"
              ...
              >
                ...
                <div class="page-info">
                  <div v-if="editingPageId === page.id" class="page-name-editor" @click.stop>
                    <input
                  ...
                  <span class="page-elements">{{ page.elements.length }} 元素</span>
                ...
                <div class="page-actions">
                  <button class="page-action-btn" @click.stop="onRenamePage(page.id)" title="重命名">
                    <i class="bi bi-pencil"></i>
                  </button>
                  <button class="page-action-btn" @click.stop="onCopyPage(page.id)" title="复制">
        ...
        <!-- 图层管理 -->
        <div v-if="store.rightPanelTab === 'layers'" class="tab-pane active" id="tabLayers" role="tabpanel">
          <div class="layers-manager" @click.self="store.clearSelection()">
            <div class="layers-toolbar">
              <button class="btn btn-sm btn-outline-secondary" @click="toggleAllVisibility">
                <i class="bi bi-eye"></i>
              </button>
              <button class="btn btn-sm btn-outline-secondary" @click="toggleAllLock">
                <i class="bi bi-lock"></i>
              </button>
            </div>

            <!-- 多选批量操作栏（仅在有选中且数量 > 1 时显示） -->
            <div v-if="store.selectedElementIds.length > 1" class="bulk-action-bar">
              <span class="bulk-count">已选中 {{ store.selectedElementIds.length }} 个</span>
              <button class="btn btn-sm btn-outline-secondary" @click="onBulkDuplicate" title="复制所有选中元素">
                <i class="bi bi-copy"></i>
                复制
              </button>
              <button class="btn btn-sm btn-outline-secondary" @click="onBulkToggleVisibility" title="显示/隐藏所有选中元素">
                <i class="bi bi-eye"></i>
                显示
              </button>
              <button class="btn btn-sm btn-outline-secondary" @click="onBulkToggleLock" title="锁定/解锁所有选中元素">
                <i class="bi bi-lock"></i>
                锁定
              </button>
              <button class="btn btn-sm btn-outline-danger" @click="onBulkDelete" title="删除所有选中元素">
                <i class="bi bi-trash"></i>
                删除
              </button>
              <button class="btn btn-sm btn-outline-secondary" @click="store.clearSelection" title="清空选择">
    ...
    <!-- 隐藏的文件选择 input -->
    <input ref="fileInputRef" type="file" accept="audio/*" style="display: none;" @change="onFileSelected">
  </aside>
</template>
...
// 页面拖拽状态
const dragPageFrom = ref<number | null>(null);
const dragPageOver = ref<number | null>(null);

// 图层拖拽状态
const dragLayerFrom = ref<string | null>(null);
const dragLayerOver = ref<string | null>(null);
...
const onHiddenModeChange = (e: Event) => {
  const target = e.target as HTMLSelectElement;
  store.setHiddenMode(target.value as 'opacity' | 'hide');
};
...
const onPageTransitionChangePerPage = (pageId: string, e: Event) => {
  ...
  if (val === '__default__') {
    ...
    store.setPageTransitionForPage(pageId, val as 'none' | 'fade' | 'slide' | 'zoom' | 'flip');
  }
};
...
const toColorPickerValue = (bgColor: string | undefined): string => {
  if (!bgColor || bgColor === 'transparent' || !bgColor.startsWith('#')) return '#ffffff';
  return bgColor;
};
...
const getElementDisplay = (element: EditorElement) => {
  ...
  if (element.type === 'group') return defaultName || `组合（${(element.children || []).length}项）`;
  return defaultName || '元素';
};

const renameLayer = (element: EditorElement) => {
  const newName = prompt('请输入图层名称', element.name || getElementDisplay(element));
...
const sortedElements = computed(() => {
  return [...(store.currentPage?.elements || [])].sort((a, b) => b.zIndex - a.zIndex);
});
...
const lastClickedLayerId = ref<string | null>(null);
...
const onLayerItemClick = (id: string, e?: MouseEvent) => {
  ...
  if (e && e.shiftKey && lastClickedLayerId.value) {
    ...
    const allIds = sortedElements.value.filter(el => !el.parentGroupId).map(el => el.id);
...
const onDesignModeChange = (e: Event) => {
  ...
  store.showToast(target.checked ? '设计模式已开启，音乐已静音' : '设计模式已关闭，音乐已恢复', 'info');
};
...
const onToggleLock = () => {
  ...
  store.showToast(`音乐位置已${store.audio.locked ? '锁定' : '解锁'}`, 'success');
};
...
const updateAudioRefFromCanvas = () => {
  if (store.audio.audioId && !audioRef.value) {
    // 优先从画布上的 <audio> 元素获取
    const el = document.getElementById(store.audio.audioId) as HTMLAudioElement | null;
    if (el) {
      audioRef.value = el;
