<template>
  <div class="editor-canvas-container">
    ...
    >
      <div class="canvas-wrapper" ref="canvasWrapperRef" :style="canvasScaleStyle">
        ...
        >
          <span class="guide-help-icon" :title="'画布尺寸 ' + store.canvasWidth + '×' + store.canvasHeight + 'px。H5 将按此尺寸渲染并等比缩放适配屏幕。'">
        ...
        >
          <span class="guide-help-icon" :title="'小屏手机安全区域（' + store.canvasWidth + '×' + store.safeAreaHeight + 'px）。上下两条虚线之间为安全区，关键内容放置在此区域可避免在小屏手机上被裁切。'">
            <i class="bi bi-question-circle-fill"></i>
          </span>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              ref="deviceScreenRef"
              class="device-screen"
              :class="[isPageTurning ? 'page-turning-' + store.pageTransition : '']"
              :style="{
                width: store.canvasWidth + 'px',
                height: store.canvasHeight + 'px',
                background: (store.currentPage?.backgroundColor && store.currentPage.backgroundColor !== 'transparent') ? store.currentPage.backgroundColor : '#ffffff',
                overflow: effectiveOverflow
              }"
            >
              <!-- 元素列表 -->
              <div
                v-for="element in sortedCanvasElements"
                :key="element.id"
              >
              ...
              >
                <div
                  ...
                  :class="{
                    ...
                    'anim-pulse': isPlaying && store.audio.animation === 'pulse',
                    'anim-bounce': isPlaying && store.audio.animation === 'bounce',
                    'anim-blink': isPlaying && store.audio.animation === 'blink',
                  }"
...
const sortedCanvasElements = computed(() =>
  [...(store.currentPage?.elements || [])].sort((a, b) => b.zIndex - a.zIndex)
);
...
const safeLineBottomStyle = computed(() => ({
  position: 'absolute' as const,
  left: (WRAPPER_PAD_X - GUIDE_LINE_OUTSET) + 'px',
  top: ((store.canvasHeight - store.safeAreaHeight) / 2 + store.safeAreaHeight + WRAPPER_PAD_Y) + 'px',
  width: (store.canvasWidth + GUIDE_LINE_OUTSET * 2) + 'px',
  height: '0px',
  pointerEvents: 'none' as const,
  zIndex: 100,
...
let marqueeWrapperRect: { left: number; top: number; width: number; height: number } | null = null;

const marqueeStyle = computed(() => {
  if (!marqueeWrapperRect) return { display: 'none' };
  const left = Math.min(marqueeStartX.value, marqueeCurrentX.value) - marqueeWrapperRect.left;
  const top = Math.min(marqueeStartY.value, marqueeCurrentY.value) - marqueeWrapperRect.top;
...
const onMarqueeStart = (e: MouseEvent) => {
  ...
  const wrapper = canvasWrapperRef.value || (e.currentTarget as HTMLElement).querySelector('.canvas-wrapper') as HTMLElement;
  if (wrapper) {
    const rect = wrapper.getBoundingClientRect();
    marqueeWrapperRect = { left: rect.left, top: rect.top, width: rect.width, height: rect.height };
  } else {
...
const onMarqueeMove = (e: MouseEvent) => {
  ...
  const dist = Math.abs(marqueeCurrentX.value - marqueeStartX.value) + Math.abs(marqueeCurrentY.value - marqueeStartY.value);
  if (dist < 3) return;
  ...
  for (let i = 0; i < elementsDom.length; i++) {
    ...
    if (rect.right < mLeft || rect.left > mRight || rect.bottom < mTop || rect.top > mBottom) continue;
...
const onAudioIconMouseDown = (e: MouseEvent) => {
  ...
  const onMove = (ev: MouseEvent) => {
    ...
    const newX = Math.max(0, Math.min(origX + dx, store.canvasWidth - store.audio.width));
    const newY = Math.max(0, Math.min(origY + dy, store.canvasHeight - store.audio.height));
...
const getPageTurnDuration = (t?: string | null) => {
  if (!t || t === 'slide' || t === 'fade' || t === 'zoom') return PAGE_TURN_FADE_MS;
  if (t === 'flip') return PAGE_TURN_FLIP_MS;
  return PAGE_TURN_NONE_MS;
};

// 切换到指定页并等待翻页动画结束，返回 device-screen 元素
const switchPageAndWait = async (pageId: string): Promise<HTMLElement | null> => {
  // 切换到目标页
...
const capturePageCanvas = async (
  ...
  const bg = options?.backgroundColor || store.pages.find((p) => p.id === pageId)?.backgroundColor || '#ffffff';
...
const effectiveOverflow = computed(() => {
  ...
  if (store.selectedElementId || (store.selectedElementIds && store.selectedElementIds.length > 0)) return 'visible';
...
const onDrop = (e: DragEvent) => {
  ...
  try {
    ...
    const deviceScreen = (e.currentTarget as HTMLElement).querySelector('.device-screen') as HTMLElement;
    const rect = deviceScreen.getBoundingClientRect();
    const x = Math.max(0, Math.min(e.clientX - rect.left - 40, store.canvasWidth - 80));
    const y = Math.max(0, Math.min(e.clientY - rect.top - 20, store.canvasHeight - 40));
