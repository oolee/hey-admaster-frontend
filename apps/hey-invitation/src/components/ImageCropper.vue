...
const props = defineProps<{
  imageUrl: string;
  initialCrop?: null | { h: number; w: number; x: number; y: number };
  initialShape?: string;
  visible: boolean;
}>();
...
const shapeTotalPages = computed(() =>
  Math.ceil(allShapes.length / SHAPES_PER_PAGE),
);
const paginatedShapes = computed(() =>
  allShapes.slice(
    shapePage.value * SHAPES_PER_PAGE,
    (shapePage.value + 1) * SHAPES_PER_PAGE,
  ),
);

const dispW = computed(() =>
  Math.round(imgNaturalW.value * displayScale.value),
);
const dispH = computed(() =>
  Math.round(imgNaturalH.value * displayScale.value),
);
...
const onImgLoad = () => {
  ...
  if (area && stage) {
    ...
    displayScale.value = Math.min(
      maxW / imgNaturalW.value,
      maxH / imgNaturalH.value,
      1,
    );
  }
  ...
  shapePage.value = allShapes.findIndex(
    (s) => s.value === (props.initialShape || 'none'),
  );
  shapePage.value =
    shapePage.value < 0 ? 0 : Math.floor(shapePage.value / SHAPES_PER_PAGE);
  ready.value = true;
};
...
const applyResize = (dx: number, dy: number) => {
  ...
  if (r === 'free') {
    ...
    if (dir.includes('w')) {
      sel.w = Math.max(MIN, ss.w - dx);
      sel.x = ss.x + dx;
    }
    if (dir.includes('s')) sel.h = Math.max(MIN, ss.h + dy);
    if (dir.includes('n')) {
      sel.h = Math.max(MIN, ss.h - dy);
      sel.y = ss.y + dy;
    }
  } else {
  ...
  <div v-if="visible" class="cropper-overlay" @click.self="emit('close')">
    <div class="cropper-modal">
      ...
      <div class="cropper-main">
        ...
        <div class="cropper-preview" ref="previewRef">
          <div class="cropper-stage" ref="stageRef">
            ...
            <div
              v-if="ready && shapeCrop !== 'none'"
              :style="shapePreviewStyle"
              class="cropper-shape-preview"
            ></div>
            <!-- crop frame -->
            <div
              v-if="ready"
            ...
            >
              ...
              <div
                class="cr-h cr-nw"
                @mousedown.stop.prevent="onHandle('nw', $event)"
              ></div>
              <div
                class="cr-h cr-ne"
                @mousedown.stop.prevent="onHandle('ne', $event)"
              ></div>
              <div
                class="cr-h cr-sw"
                @mousedown.stop.prevent="onHandle('sw', $event)"
              ></div>
              <div
                class="cr-h cr-se"
                @mousedown.stop.prevent="onHandle('se', $event)"
              ></div>
              <!-- edges -->
              <template v-if="ratio === 'free'">
                <div
                  class="cr-h cr-n"
                  @mousedown.stop.prevent="onHandle('n', $event)"
                ></div>
                <div
                  class="cr-h cr-s"
                  @mousedown.stop.prevent="onHandle('s', $event)"
                ></div>
                <div
                  class="cr-h cr-e"
                  @mousedown.stop.prevent="onHandle('e', $event)"
                ></div>
                <div
                  class="cr-h cr-w"
                  @mousedown.stop.prevent="onHandle('w', $event)"
                ></div>
              </template>
            </div>
          </div>
        ...
        <div class="cropper-options">
          <div class="cr-section">
            ...
            <div class="cr-ratio-grid">
              <button
                ...
                :class="
                  ratio === r.value ? 'btn-primary' : 'btn-outline-secondary'
                "
          ...
          <div class="cr-section">
            ...
            <div class="cr-pagination" v-if="shapeTotalPages > 1">
              ...
              <button
                class="btn btn-sm btn-outline-secondary"
                :disabled="shapePage >= shapeTotalPages - 1"
                @click="shapePage++"
              >
                &raquo;
              </button>
      ...
      <div class="cropper-footer">
        ...
        <button class="btn btn-primary" @click="onConfirm" :disabled="!ready">
          确认
        </button>
      </div>
    </div>
