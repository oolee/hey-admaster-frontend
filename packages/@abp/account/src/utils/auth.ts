...
import { useAppConfigExtended } from '@abp/core';
...
import rawSecureLS from 'secure-ls';

// ------------------------------
// 👇 这一段就是你给的“万能兼容方案”
// 专门解决 secure-ls 2.x export = SecureLS 导致的所有报错

// 2. 定义构造函数类型（名字和变量区分开）
type SecureLSClass = new (config?: {
  encodingType?: string;
...
// 3. 三层兼容逻辑
const secureLSModule = rawSecureLS as unknown as {
  default?: SecureLSClass;
  SecureLS?: SecureLSClass;
};

// 4. 变量名也换一下，避免和类型重名
const SecureLSInstanceConstructor =
  secureLSModule.default ??
  secureLSModule.SecureLS ??
  (rawSecureLS as unknown as SecureLSClass);
...
class AbpUserManager extends UserManager {
  ...
  override async signinResourceOwnerCredentials(
    ...
    const client_secret = this.settings.client_secret;
    if (!client_secret) {
      logger.error('A client_id/client_secret is required');
      throw new Error('A client_id is required');
    }
    const body = new URLSearchParams({
...
const { auth } = useAppConfigExtended(import.meta.env, import.meta.env.PROD);
...
const ls = new SecureLSInstanceConstructor({
  encodingType: 'aes',
