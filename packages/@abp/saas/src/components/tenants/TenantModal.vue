...
const defaultModel: TenantFormDto = {
  name: '',
  isActive: true,
  useSharedDatabase: true,
  connectionStrings: [],
...
const tenant = ref<TenantFormDto>({ ...defaultModel });
...
async function onGet() {
  ...
  try {
    ...
    tenant.value = { ...defaultModel, ...editionDto };
  } finally {
...
async function onSubmit() {
  try {
    ...
    const api = tenant.value.id
      ? updateApi(tenant.value.id, tenant.value as unknown as TenantUpdateDto)
...
function onConnectionChange(data: TenantConnectionStringDto) {
  return new Promise<void>((resolve) => {
    ...
    const connectionString = tenant.value.connectionStrings.find(
